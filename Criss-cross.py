...
def checkifwon():
    ...
    winner_1 = False
    winner_2 = False
    # в указанных ниже двух переменных хранятся номера клеток для выбора игроком X и игроком O 
    cells_for_cross_choose_list, cells_for_zero_choose_list
# происходит проверка: после выбора клетки появился ли горизонтальный ряд из клеток с одинаковым знаком
    def find_horizontal_row_of_5(lst_var):
        #составляется список из чисел, соотвествующих номерам клеток с крестиками или ноликами (в зависимости от указанной переменной при вызове функции)
        numbers_list = []
        for i in lst_var:
            s = re.findall(r'\d+', i)
        ...
        for m in range(len(array_board)):
            # объявляется массив от 0 до 100, в котором числа в списках отсортированы по горизонтали 
            row_list = array_board[[m], :]
            lst_result = []
            lst_result_set = set()
            # в этом цикле происходит поиск по номерам ячеек, которые были уже заняты знаком походившего участника партии. Результат записывается в lst_result
            for i in row_list:
                for n in i:
                    for k in sorted_numbers_lst:
                        if n == k:
                            lst_result.append(k)
            # в этом цикле происходит поиск по двум идущим подряд номерам ячеек, при обнаружении во множество lst_result_set записываются оба числа
            ...
            '''
            если длина множества 5 и более элементов, то происходит проверка на наличие 5 подряд идущих номеров с одинаковыми знаками. 
            Для охвата всех вариантов проверка на 5 идущих подряд элементов происходит как с начала списка, так и с последнего элемента в обратном направлении.
            Если 5 номеров найдены, то засчитывается поражение ходившему игроку или компьютеру.
            ...
            if len(lst_result_set) >= 5:
                ...
                if final_list[-5] + 4 == final_list[-4] + 3 == final_list[-3] + 2 == final_list[-2] + 1 == final_list[-1]:
                    winner = True
                    # эта функция окрашивает ячейки найденного ряда в красный цвет и белый шрифт
                    ...
                    change_color_of_defeat_row(final_list[:5])
                    return winner
    # происходит проверка: после выбора клетки появился ли вертикальный ряд ряд из клеток с одинаковым знаком. Алгоритм тот же, что и у функции поиска горизонтального ряда
        ...
        for m in range(len(array_board)):
            ...
            if len(lst_result_set) >= 5:
                ...
                if final_list[-5] + 40 == final_list[-4] + 30 == final_list[-3] + 20 == final_list[-2] + 10 == final_list[-1]:
                    ...
                    break
    '''
    Происходит проверка: после выбора клетки появился ли ряд диагоналей из клеток с одинаковым знаком.
    В данной функции проверяется диагональ слева направо, проверка идет по правой половине массива
    Алгоритм тот же, что и у функции поиска горизонтального ряда.
    ...
    '''
    Происходит проверка: после выбора клетки появился ли ряд диагоналей из клеток с одинаковым знаком.
    В данной функции проверяется диагональ слева направо, проверка идет по левой половине массива
    Алгоритм тот же, что и у функции поиска горизонтального ряда.
    ...
    '''
    Происходит проверка: после выбора клетки появился ли ряд диагоналей из клеток с одинаковым знаком.
    В данной функции проверяется диагональ справа налево, проверка идет по левой половине массива
    Алгоритм тот же, что и у функции поиска горизонтального ряда.
    ...
    '''
    Происходит проверка: после выбора клетки появился ли ряд диагоналей из клеток с одинаковым знаком.
    В данной функции проверяется диагональ справа налево, проверка идет по правой половине массива
    Алгоритм тот же, что и у функции поиска горизонтального ряда.
        ...
        for m in range(len(array_board)):
            ...
            if len(lst_result_set) >= 5:
                ...
                if final_list[-5] + 36 == final_list[-4] + 27 == final_list[-3] + 18 == final_list[-2] + 9 == final_list[-1]:
                    ...
                    change_color_of_defeat_row(final_list[:5])
                    return winner
    # здесь запускаются все указанные выше функции с проверкой хода и игрока X и игрока O. Если хотя бы одна из функций возвращает True, то победителем объявляется противник проверяемого игрока
...
def lists_for_computer_move():
    ...
    # это список с числами - номерами незанятых клеток
    numbers_empty_list = []
    for i in empty_cells_list:
        s = re.findall(r'\d+', i)
        numbers_empty_list.append(int(s[0]))
    # это список с числами - номерами клеток, которые не заняты O
    numbers_zero_list = []
    for i in cells_for_zero_choose_list:
        s = re.findall(r'\d+', i)
        numbers_zero_list.append(int(s[0]))
    # это список с числами - номерами клеток, которые не заняты X
    numbers_cross_list = []
    for i in cells_for_cross_choose_list:
        s = re.findall(r'\d+', i)
        numbers_cross_list.append(int(s[0]))
    # выбираем какой список для выбора достанется компьютеру в зависимости от полученного им знака: X или O
    if computer_side == "X":
...
def arr_for_choice():
    ...
    lst_result = []
    for m in range(len(array_board)):
        # в начале цикла объявляются переменные, в которых хранятся ряды массива от 0 до 100
        
        # горизонтальные ряды
        row_array = array_board[[m], :]
        # вертикальные ряды
        column__array = array_board[:, [m]]
        # ряды диагоналей слева направо по правой стороне массива
        diagonal_up_array = np.diag(array_board, k=m)
        # ряды диагоналей слева направо по левой стороне массива
        diagonal_down_array = np.diag(array_board, k=-m)
        # ряды диагоналей справа налево по левой стороне массива
        reverse_diagonal_up_array = np.fliplr(array_board).diagonal(m)
        # ряды диагоналей справа налево по правой стороне массива
        reverse_diagonal_down_array = np.fliplr(array_board).diagonal(-m)
        lst_before_result = []
        
        # в этой функции идет поиск клеток, которые заняты знаком игрока в горизонтальных и вертикальных рядах
        ...
        # в этой функции идет поиск клеток, которые заняты знаком игрока в рядах диагоналей
            ...
            for i in numbers_arr:
                for k in sorted_list_for_choose:
                    if i == k:
                        list_var.append(k)
            return lst_before_result.append(list_var)
        # поиск доступных для выбора клеток по всем казанным рядам и запись их в деиный список lst_before_result
        ...
        find_not_enemy_numbers_of_cells_for_diagonals(
            reverse_diagonal_up_array)
        find_not_enemy_numbers_of_cells_for_diagonals(
            reverse_diagonal_down_array)
        for i in lst_before_result:
            lst_result.append(i)
    chosen_cell = ""
    # переменная, которой присваивается случайно выбранное число из списка с пустыми клетками
    choose_number = random.choice(numbers_empty_list)
    # мы получили список lst_result, в котором вложены списки номеров тех клеток, в которые уже походил совершающий сейчас ход игрок.
    # в sorted_for_len_lst_result эти списки сортируются по длине по возрастанию
    sorted_for_len_lst_result = sorted(lst_result, key=len, reverse=True)
    
    number_of_tries = 0
    list_for_check = []
    # в этой функции происходит проверка на наличие списков, то есть рядов, в которых уже есть 4 клетки, которые заняты знаком игрока. Результат записывается в list_for_check
    ...
    """
    В функции check_for_lose_streak() происходит проверка на то, что приведет ли к поражению выбор ячейки сделанный в переменной choose_number.
    Если такая последовательность найдена, то к переменной danger_streak_response прибавляется единица, итогое число и возвращает функция. Т оесть если всё ок, то она вернет 0.
    Алгоритм поиска схож с функцией определения победителя.
                        ...
                        for i in range(len(sorted_check_lst)):
                            if i + 4 < len(sorted_check_lst):
                                if (sorted_check_lst[i] + 4 == sorted_check_lst[i+1] + 3 == sorted_check_lst[i+2] + 2 == sorted_check_lst[i+3] + 1 == sorted_check_lst[i+4]) or (sorted_check_lst[-5] + 4 == sorted_check_lst[-4] + 3 == sorted_check_lst[-3] + 2 == sorted_check_lst[-2] + 1 == sorted_check_lst[-1]):
                                    check_row_array_for_danger_streak.append(
                                        sorted_check_lst[i])
                                    check_row_array_for_danger_streak.append(
                                        sorted_check_lst[i+1])
                                    check_row_array_for_danger_streak.append(
                                        sorted_check_lst[i+2])
                                    check_row_array_for_danger_streak.append(
                                        sorted_check_lst[i+3])
                                    check_row_array_for_danger_streak.append(
                                        sorted_check_lst[i+4])
                                    for m in range(len(array_board)):
                                        row_list = array_board[[m], :]
                                    ...
                                    check_column_array_for_danger_streak.append(
                                        sorted_check_lst[i])
                                    check_column_array_for_danger_streak.append(
                                        sorted_check_lst[i+1])
                                    check_column_array_for_danger_streak.append(
                                        ...
                                        for j in column_list:
                                            for n in j:
                                                for s in check_column_array_for_danger_streak:
                                                    if n == s:
                                                        lst_result_array_column.append(
                                                            s)
                                    ...
                                    check_diagonal_array_for_danger_streak.append(
                                        sorted_check_lst[i])
                                    check_diagonal_array_for_danger_streak.append(
                                        sorted_check_lst[i+1])
                                    check_diagonal_array_for_danger_streak.append(
                                    ...
                                    for m in range(len(array_board)):
                                        ...
                                        if (check_diagonal_array_for_danger_streak == lst_result_array_diagonal_list_up) and (len(check_diagonal_array_for_danger_streak) >= 5 and len(lst_result_array_diagonal_list_up) >= 5):
                                            ...
                                            danger_streak_response += 1
                                    for m in range(len(array_board)):
                                        diagonal_list_down = np.diag(
                                            array_board, k=-m)
                                        lst_result_array_diagonal_list_down = []
                                        for i in diagonal_list_down:
                                            for s in check_diagonal_array_for_danger_streak:
                                                if i == s:
                                                    lst_result_array_diagonal_list_down.append(
                                                        s)
                                    ...
                                    check_reversed_diagonal_array_for_danger_streak.append(
                                        sorted_check_lst[i])
                                    check_reversed_diagonal_array_for_danger_streak.append(
                                        sorted_check_lst[i+1])
                                    check_reversed_diagonal_array_for_danger_streak.append(
                                    ...
                                    check_reversed_diagonal_array_for_danger_streak.append(
                                        sorted_check_lst[i+4])
                                    for m in range(len(array_board)):
                                        reverse_diagonal_list_up = np.fliplr(
                                            array_board).diagonal(m)
                                        lst_result_array_reverse_diagonal_list_up = []
                                        for i in reverse_diagonal_list_up:
                                            for s in check_reversed_diagonal_array_for_danger_streak:
                                                if i == s:
                                                    lst_result_array_reverse_diagonal_list_up.append(
                                                        s)
                                        if (check_reversed_diagonal_array_for_danger_streak == lst_result_array_reverse_diagonal_list_up) and (len(check_reversed_diagonal_array_for_danger_streak) >= 5 and len(lst_result_array_reverse_diagonal_list_up) >= 5):
                                            ...
                                            danger_streak_response += 1
                                    for m in range(len(array_board)):
                                        reverse_diagonal_list_down = np.fliplr(
                                            array_board).diagonal(-m)
                                        lst_result_array_array_for_danger_streak_down = []
                                        for i in reverse_diagonal_list_down:
                                            for s in check_reversed_diagonal_array_for_danger_streak:
                                                if i == s:
                                                    lst_result_array_array_for_danger_streak_down.append(
                                                        s)
    ...
    # в этой переменной записываются те числа, которые уже не подходят, так как могут привести к создранию ряда из 5 клеток и, соответственно, поражению
    used_numbers = (set([]))

    check_for_lose_streak()

    """
    В данной функции происходит проверка выбранной клетки на поражение с вызовом check_for_lose_streak().
    Если проверка не пройдена, то выбирается новая клетка, пока не найден будет безопасный вариант.
    Если в ходе 10 попыток вариант не найден, то возвращается номер последней выбранной клетки.
    """
    def check_response():
        global number_of_tries, choose_number
        while True:
...
def change_color_of_defeat_row(var):
    for i in var:
        lst_buttons_variables_names['cell_' +
                                    str(i)].config(bg="#ac1818", fg='white')
...
def greetings_for_winner():
    ...
    positionRight = int(
        greetings_window.winfo_screenwidth()/1.61 - windowWidth/2)
    positionDown = int(
        greetings_window.winfo_screenheight()/2 - windowHeight/2)
    greetings_window.geometry("+{}+{}".format(positionRight, positionDown))
    buttonFont = font.Font(size=16)
    button_no = Button(greetings_window, text="Нет", bg="red", fg="white", command=lambda: [
                       greetings_window.destroy(), root.destroy()], width=10, height=1)
    button_yes = Button(greetings_window, text="Да", bg="green", fg="white", command=lambda: [
                        greetings_window.destroy(), reset()], width=10, height=1)
...
def b_click(b):
    ...
    if b["text"] == " " and selected_player == 1:
        ...
        disable_all_buttons()
        messagebox.showerror("Обратные крестики-нолики",
                             "Это поле уже было выбрано \n Выберите другую ячейку", parent=root)
        enable_all_buttons()
...
def set_board():
    ...
    for i in range(1, 101):
        name = "cell_%s" % i
        lst_buttons_variables_names[name] = Button(root, text=" ", font=(
            "Helvetica", 20), heigh=1, width=2, bg="#a5a8a6", command=lambda: b_click(lst_buttons_variables_names[name]))
    count_row = 0
    count_column = 0
    ...
    for key, value in lst_buttons_variables_names.items():
        ...
        if count_column == 0:
            new_var_button.grid(
                row=count_row, column=count_column, padx=(38, 0))
        else:
...
def popup_window():
    ...
    cross = Radiobutton(window, text="X", variable=var,
                        value=1, command=choose, width=10, height=1)
    zero = Radiobutton(window, text="O", variable=var,
                       value=2, command=choose, width=10, height=1)
    button_close = Button(window, text="Выбрать", command=lambda: [
                          callback(), window.destroy(), set_board()], state=DISABLED, width=10, height=1)
